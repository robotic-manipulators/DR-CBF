...
def run_experiment(step_size, vision_noise, use_dr=True, enable_brown_spheres=True,
                   plot_objects=None, **kwargs):
    ...
    unc_scale = kwargs.get('unc_scale', config.UNCERTAINTY_SCALE)
    ...
    if plot_objects is not None:
        fig, ax, lines = plot_objects
        dist_history = [[] for _ in range(num_obs)]
        step_history = []
        ...
        dist_history = step_history = None
    ...
    while True:
        ...
        if plot_objects is not None:
            dist_current = []
            for i in range(num_obs):
                closest_points = p.getClosestPoints(bodyA=robot, bodyB=obstacle_ids[i], distance=1.0)
                if len(closest_points) > 0:
                    min_dist = min(point[8] for point in closest_points)
                else:
                    min_dist = 10.0
                dist_current.append(min_dist)
            step_history.append(step_counter)
            for i in range(num_obs):
                dist_history[i].append(dist_current[i])

            if step_counter % plot_update_interval == 0:
                for i, line in enumerate(lines):
                    line.set_data(step_history, dist_history[i])
                ax.set_xlim(0, max(step_history) + 10)
                fig.canvas.draw_idle()
                plt.pause(0.001)

        if step_counter % 5 == 0:
            if enable_brown_spheres:
                for bid in brown_sphere_ids:
                    p.removeBody(bid)
                brown_sphere_ids.clear()
            ...
            for i in range(num_obs):
                ...
                obs_uncertainty_radius[i] = unc_scale * sigma

                p.resetBasePositionAndOrientation(viz_obs_ids[i], obs_est_pos[i], [0, 0, 0, 1])

                if enable_brown_spheres and use_dr:
                    brown_vis = p.createVisualShape(p.GEOM_SPHERE,
                                                    radius=obs_radius + obs_uncertainty_radius[i],
                                                    rgbaColor=config.BROWN_COLOR)
                    brown_body = p.createMultiBody(baseMass=0,
                                                   baseVisualShapeIndex=brown_vis,
                                                   basePosition=obs_est_pos[i])
                    brown_sphere_ids.append(brown_body)
    ...
    if plot_objects is not None:
        return result, step_history, dist_history
    else:
        return result